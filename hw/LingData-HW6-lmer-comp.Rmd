...
```{r 1.1, echo = FALSE}
ggplot(upsid, aes(consonants, vowels)) + geom_point()
```
...
```{r 1.5, echo=FALSE}
ggplot(upsid, aes(consonants, vowels)) + geom_point() + geom_line(data = augment(fit1), aes(x = upsid.consonants, y = .fitted), color = "blue")
```
...
```{r 2.1, echo=FALSE}
ggplot(upsid, aes(consonants, vowels, color=upsid$area)) + geom_point()
...
```{r 2.3, echo=FALSE}
upsid$model2 <- predict(fit2)

upsid %>% 
  ggplot(aes(consonants, vowels, color=area))+
  geom_point()+
  geom_line(aes(consonants, model2, color = area))+
...
Consonants variable is still significant. After we added area random effect p-value of consonants variable became bigger. The lines direction remained the same, all the lines are parallel because only intercepts varied.
```
...
```{r 3.2, echo=FALSE}
upsid$model3 <- predict(fit3)

upsid %>% 
  ggplot(aes(consonants, vowels, color=area))+
  geom_point()+
  geom_line(aes(consonants, model3, color = area))+
  labs(x = "number of consonants",
       y = "number of vowels",
       caption = "data from LAPSyD")
```
...
```{3.3}
# Consonants are not significant now. And the lines are not parallel either, because slopes are random now. 
```
...
```{r 4.2, echo=FALSE}
upsid$model4 <- predict(fit4)

upsid %>% 
  ggplot(aes(consonants, vowels, color=area))+
  geom_point()+
  geom_line(aes(consonants, model4, color = area))+
  labs(x = "number of consonants",
       y = "number of vowels",
       caption = "data from LAPSyD")
```
...
```{4.3}
Consonants are not significant in this model. The lines are different because in model 2 onlu intercepts were varying, in model 3 only slopes were varying and in model 4 both slopes and intercepts were varying.
```
